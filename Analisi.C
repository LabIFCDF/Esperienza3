...
void Analisi()
{
	...
	TH1D* h1_s =  new TH1D("h1_s", "Tempo_s",200 ,0,0);
	TH1D* h2_s =  new TH1D("h2_s", "Pmt 1_s",200 ,0,0);
	TH1D* h3_s =  new TH1D("h3_s", "Pmt 2_s",200 ,0,0);	
	TH1D* h1_c =  new TH1D("h1_c", "Tempo_c",200 ,0,0);
	TH1D* h2_c =  new TH1D("h2_c", "Pmt 1_c",200 ,0,0);
	TH1D* h3_c =  new TH1D("h3_c", "Pmt 2_c",200 ,0,0);	
	TH1D* h1_d =  new TH1D("h1_d", "Tempo_d",200 ,0,0);
	TH1D* h2_d =  new TH1D("h2_d", "Pmt 1_d",200 ,0,0);
	TH1D* h3_d =  new TH1D("h3_d", "Pmt 2_d",200 ,0,0);
	...
	TH1D* ht1 =  new TH1D("ht1", "dt1a-dt1b",200 ,0,0);	
	TH1D* ht2 =  new TH1D("ht2", "dt2a-dt2b",200 ,0,0);	
	...
	TCanvas* c3= new TCanvas("c3", "Velocità luce nella sbarra", 2000,500);
	...
	//Riempio i vector
    	...
    	for(int i = 0; i < n; i++)
    	{
    		Pmt1_s[i]=Pmt1_s[i]*40;
    		Pmt2_s[i]=Pmt2_s[i]*40;
    		Pmt1_c[i]=Pmt1_c[i]*20;
    		Pmt2_c[i]=Pmt2_c[i]*20;
    		Pmt1_d[i]=Pmt1_d[i]*40;
    		Pmt2_d[i]=Pmt2_d[i]*40;
	...
	for(int i = 0; i < n; i++)
    	...
    	//calcolo il tempo totale di attraversamento
    	for(int i = 0; i < n; i++)
    	{
    		ttot[i]=(Pmt2_s[i]-Pmt2_d[i]);
